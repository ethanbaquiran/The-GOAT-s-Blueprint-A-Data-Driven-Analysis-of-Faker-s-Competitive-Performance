...
The dataset contains one row per player per game. For Faker's individual rows, the most relevant columns are:
...
- **5 rows dropped** from the 2025 dataset where `golddiffat25` was missing (games ending before 25 minutes), when using 25-minute features.

The resulting dataset has **187 games from 2015** and a comparable sample from 2025. Below is the head of the cleaned DataFrame:

| kda | golddiffat15 | damageshare | visionscore | result | year |
|----:|-------------:|------------:|------------:|-------:|-----:|
| 3.000 | 312 | 0.312 | 28 | 1 | 2015 |
| 1.500 | -204 | 0.271 | 19 | 0 | 2015 |
| 12.000 | 887 | 0.391 | 34 | 1 | 2015 |
| 2.000 | -511 | 0.248 | 22 | 0 | 2015 |
| 9.000 | 623 | 0.365 | 31 | 1 | 2015 |
...
|-----:|-------:|----:|-------------:|------------:|-----------:|----:|------------:|
| 2015 | 0 | 1.730 | -358.2 | 0.261 | -412.1 | 312.4 | 19.3 |
| 2015 | 1 | 9.320 | 502.8 | 0.341 | 531.7 | 408.9 | 27.1 |
...
We tested whether the missingness of `golddiffat25` in the 2025 dataset depends on `result` (win/loss).

**Null Hypothesis:** The missingness of `golddiffat25` is independent of match outcome.
...
This baseline confirms that KDA and gold difference carry useful signal. However, it relies on only two dimensions of performance, and an unconstrained decision tree is prone to overfitting. These two weaknesses motivate the final model.
...
The final model improved on the baseline by ~13 percentage points on test accuracy. The key improvements were: (1) adding DPM, damage share, and vision score as features; (2) using a Random Forest instead of a single Decision Tree, reducing variance through ensembling; and (3) grid search with cross-validation to prevent overfitting.
